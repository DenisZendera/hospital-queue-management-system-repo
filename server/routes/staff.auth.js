 ...
 * Logout staff/doctor and set online status to offline
 */
router.post('/logout', async (req, res) => {
  try {
    ...
    // Set is_online to 0 when staff/doctor logs out
    await run(
      'UPDATE users SET is_online = 0 WHERE user_id = ? AND role IN (?, ?, ?)',
      [userId, 'doctor', 'admin', 'superadmin']
    );
