...
router.post('/book', verifyPatientToken, async (req, res) => {
  try {
    ...
    // ✅ PREVENT DUPLICATE BOOKINGS: Check if patient already has any active appointment
    ...
    const existingAppointments = await query(
      `SELECT appointment_id, appointment_date, appointment_time, status
       FROM appointments
       WHERE patient_id = ? AND status NOT IN ('cancelled', 'no-show', 'completed')
       ORDER BY appointment_date ASC, appointment_time ASC`,
      [patientId]
    );

    if (existingAppointments.length > 0) {
      console.log('  ✗ Active appointment already exists!', existingAppointments[0]);
      const existing = existingAppointments[0];
      return res.status(400).json({
        success: false,
        message: `You already have an active appointment scheduled for ${existing.appointment_date} at ${existing.appointment_time}. Please complete or cancel your current appointment before booking a new one.`,
        error: 'ACTIVE_APPOINTMENT_EXISTS',
        existingAppointment: {
    ...
    console.log('  ✓ No active appointments found - patient can book');
...
router.post('/emergency', verifyStaffToken, async (req, res) => {
  try {
    ...
    // Get ALL appointments for this doctor on this date (including completed ones)
    const allAppointments = await query(`
      SELECT appointment_id, queue_number, appointment_time, patient_id, status
      FROM appointments
      WHERE doctor_id = ?
    ...
    // Find the first non-completed appointment position
    const firstNonCompletedIndex = allAppointments.findIndex(apt => apt.status !== 'completed');

    // Find the last completed appointment
    let lastCompletedIndex = -1;
    for (let i = allAppointments.length - 1; i >= 0; i--) {
      if (allAppointments[i].status === 'completed') {
        lastCompletedIndex = i;
        break;
      }
    }

    let emergencyQueuePosition;
    let emergencyTimeSlot;

    if (firstNonCompletedIndex === -1) {
      // All appointments are completed - add emergency at the end
      emergencyQueuePosition = allAppointments.length > 0 ? allAppointments[allAppointments.length - 1].queue_number + 1 : 1;
      emergencyTimeSlot = calculateEmergencyTimeSlot(emergencyQueuePosition);
      console.log(`📋 All appointments completed. Adding emergency at position ${emergencyQueuePosition}, time ${emergencyTimeSlot}`);
    } else if (lastCompletedIndex === -1) {
      // No completed appointments - insert at the very beginning
      emergencyQueuePosition = allAppointments[0].queue_number;
      emergencyTimeSlot = allAppointments[0].appointment_time;
      console.log(`📋 No completed appointments. Inserting emergency at position ${emergencyQueuePosition}, time ${emergencyTimeSlot}`);
      console.log(`📋 This will push back ${allAppointments.length} appointments`);
    } else {
      // There are completed appointments - insert RIGHT AFTER the last completed one
      // The emergency takes the queue position AFTER the last completed appointment
      emergencyQueuePosition = allAppointments[lastCompletedIndex].queue_number + 1;
      emergencyTimeSlot = calculateEmergencyTimeSlot(emergencyQueuePosition);
      console.log(`📋 Last completed: Queue ${allAppointments[lastCompletedIndex].queue_number} at ${allAppointments[lastCompletedIndex].appointment_time}`);
      console.log(`📋 Inserting emergency at position ${emergencyQueuePosition}, time ${emergencyTimeSlot}`);
      console.log(`📋 This will push back ${allAppointments.length - lastCompletedIndex - 1} non-completed appointments`);
    }

    // Get only non-completed appointments that need to be pushed back
    const appointmentsToPushBack = await query(`
      ...
      WHERE doctor_id = ?
        ...
        AND queue_number >= ?
      ORDER BY queue_number ASC
    `, [doctorId, finalDate, emergencyQueuePosition]);

    console.log(`📋 Found ${appointmentsToPushBack.length} appointments to push back`);

    // Push back all non-completed appointments by 1 position (25 minutes)
    for (const apt of appointmentsToPushBack) {
      const newQueueNumber = apt.queue_number + 1;
      const newTimeSlot = calculateEmergencyTimeSlot(newQueueNumber);

      console.log(`  → Pushing appointment ${apt.appointment_id}: queue ${apt.queue_number} → ${newQueueNumber}, time ${apt.appointment_time} → ${newTimeSlot}`);
    ...
    // Insert emergency appointment at the calculated position
    console.log(`✓ Inserting emergency patient at queue position ${emergencyQueuePosition}, time ${emergencyTimeSlot}, status: scheduled`);
    ...
    const result = await run(`
      INSERT INTO appointments (
        ...
        queue_number,
        created_at
      ) VALUES (?, ?, ?, ?, 'scheduled', ?, ?, ?, datetime('now', '+2 hours'))
    `, [
      patientId,
      doctorId,
      finalDate,
    ...
    console.log(`   - Queue Position: ${emergencyQueuePosition}`);
    console.log(`   - Appointment Time: ${emergencyTimeSlot}`);
    console.log(`   - Patients pushed back: ${appointmentsToPushBack.length}`);
    console.log('='.repeat(60));

    res.status(201).json({
      success: true,
      message: `Emergency patient ${patientInfo[0].full_name} added to queue. ${appointmentsToPushBack.length} patient(s) pushed back by 25 minutes.`,
      data: {
        appointmentId: result.lastID,
        queueNumber: emergencyQueuePosition,
        appointmentTime: emergencyTimeSlot,
        patientName: patientInfo[0].full_name,
        appointmentDate: finalDate,
        patientsPushedBack: appointmentsToPushBack.length
      }
    });
