...
import { api, patientAuth } from "@/lib/api";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
const ProfileSettings = () => {
  ...
  const handleLogout = async () => {
    try {
      // Call backend to set is_online = 0
      ...
      // Still log out from frontend even if backend call fails
      toast.success("Logged out successfully");
      window.location.replace('/');
    }
  };
  ...
  const handleDeleteAccount = async () => {
    try {
      ...
      if (response.success) {
        ...
        // Force logout and redirect to login page with full page reload
